...
*   **타임아웃 설정**: `asyncio.wait_for(..., timeout=settings.ai_timeout_seconds)`를 적용하여 지정된 시간 내에 응답이 도착하지 않을 경우 즉시 코루틴을 취소하고 `AI_TIMEOUT` 예외를 발생시킵니다.
*   **재시도 전략 및 Rate Limit 방어 (서킷 브레이커 도입)**:
    *   **서버 레벨 자동 재시도 금지**: AI 응답 지연 시 서버에서 무리하게 자동 재시도를 반복하면 429(Rate Limit) 페널티 누적 및 서버 자원 고갈이 발생하므로, 백엔드는 내부 재시도 없이 즉각 에러 코드를 반환합니다.
    *   **글로벌 서킷 브레이커(차단기) 로직**: 구글 API에서 429(Quota Exceeded) 한도 초과 에러 발생 시, 응답에 포함된 대기 시간(retry in Xs)에 5초의 안전 버퍼를 더해 서버 메모리에 글로벌 락(Global Lock)을 설정합니다. 잠금 기간 내 들어오는 모든 유저의 요청은 외부 API 호출 없이 백엔드에서 0.001초 만에 차단(Fast-fail)되어 서비스 마비를 완벽히 방어합니다.
    *   **클라이언트 레벨**: 프론트엔드 UI는 별도의 재시도 기능을 두지 않으며, 에러 응답을 받은 사용자가 직접 질문을 다시 입력하는 방식으로 재시도를 위임합니다.
*   **대체 응답 정책 (Fallback Response)**:
    *   타임아웃 시: `"현재 응답이 지연되고 있어요. 잠시 후 다시 시도해 주세요. (error: AI_TIMEOUT)"`
    *   한도 초과(429) 시: `"현재 이용량이 많아 요청 한도를 초과했습니다. 약 1분 뒤에 다시 시도해 주세요."`
    *   기타 서버 에러 시: `"일시적인 오류가 발생했습니다. 잠시 후 다시 시도해 주세요."`
    *   모든 사용자 질문은 AI 호출 전 **1차 DB에 선저장(Commit)**되므로, 외부 AI 서버 오류나 예기치 못한 종료 시에도 소중한 프롬프트 데이터는 영구 보존됩니다.
